...
router.post('/seller/products', verifyAuthToken, (req, res) => {
	var sql = `SELECT products.id AS id, products.catid AS catId, categories.name AS catName, products.name AS prodName, price, stock, sold`
	if(!req.body.allProducts)
		sql += `, description`;
	sql += ` FROM products LEFT JOIN categories ON products.catid = categories.id WHERE sid = ${req.id}`;
	if(!req.body.allProducts)
		sql += ` AND products.id = ${req.body.pid}`;
	sql += ` ORDER BY id`;
	conn.query(sql, function (err, rows) {
		if (err) {
			var reply = { result: "error", code: err.code, msg: err.sqlMessage };
...
router.post('/seller/modify', verifyAuthToken, (req, res) => {
	insArr = [req.id];
	with(req.body)
		insArr.push(category, prodName, price, stock, description, pid);
	console.log(insArr);
	var sql = 'UPDATE products SET sid = ?, catid = ?, name = ?, price = ?, stock = ?, description = ? WHERE id = ?';
	conn.query(sql, insArr, function (err, result) {
		if (err) {
			var reply = { result: "error", code: err.code, msg: err.sqlMessage };
			res.json(reply);
...
router.get('/seller/select', verifyAuthToken, (req, res) => {
	var sql = `SELECT name_owner, phone, email, dob, sex, name_business, shop, area, landmark, city, state, pin, gstin from sellers WHERE id = ${req.id}`;
	conn.query(sql, function (err, rows) {
		if (err) {
			var reply = { result: "error", code: err.code, msg: err.sqlMessage };
			res.status(500).json(reply);
			...
			var reply = { result: "success", row: rows[0]};
			res.status(200).json(reply);
			// console.log(rows);
		}
	});
...
router.post('/seller/update', verifyAuthToken, (req, res) => {
	with(req.body) {
		var insArr = [phone, email, busName, gstin];
		with(address) {
			insArr.push(shop, area, landmark, city, state, pin);
		}
	}
	insArr.push(req.id);
	console.log(insArr);
	var sql = 'UPDATE sellers SET phone = ?, email = ?, name_business = ?, gstin = ?, shop = ?, area = ?, landmark = ?, city = ?, state = ?, pin = ?';
	if(req.body.passwords.pass) {
		sql += ', passhash = ?';
		insArr.push(bcrypt.hashSync(req.body.passwords.pass, 10));
	}
	sql += ' WHERE id = ?';
